const main = () => {
  ...
  if (!sheet) return;
  const getSheetTime = sheet.getRange(1, 1).getValue(); // 前回の更新日時をSpreadSheetから取得
  const getSheetTimeIfErr = sheet.getRange(1, 1).getDisplayValue(); // エラーがあった場合のために文字列のまま前回の更新日時をSpreadSheetから取得
  ...
  try {
    ...
    if (myMessages.length !== 0) {
      ...
      const duplicateMain = setValArray.filter(
        (data) => Date.parse(getSheetTime) <= Date.parse(data[0])
      ); // SpreadSheetの日付よりも新しいものだけ、Fwdの文字列がないものを反映
      const validateSetValArry = duplicateMain.filter(
        (ary) => Object.keys(ary).length
    ...
    sheet.getRange(1, 1).setValue(fomattedDate); // 前回の更新日時を更新しSpreadSheetへ反映
  } catch (e) {
    // エラー時の処理を書く

    console.log(e);
    // SpreadSheetの更新年月日時間をエラーで止まったときの時間のままにする（一応更新しておく）
    sheet.getRange(1, 1).setValue(getSheetTimeIfErr); // 前回の更新日時を更新しSpreadSheetへ反映
  }
}; // const main = () => {
